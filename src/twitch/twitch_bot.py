...
class TwitchBot(commands.Bot):
    def __init__(self, configuration: Configuration, log: Log, music_controller: MusicController):
    ...
    async def event_command_error(self, context: commands.Context, error: Exception) -> None:
        if isinstance(error, commands.errors.CommandOnCooldown):
            ...
            resp = self.configuration.locate.translate(error.clearance, command=context.command.name)
            await context.reply(resp)
            self.log.resp(resp)
            ...
            resp = self.configuration.locate.translate('USER_BANNED')
            await context.reply(resp)
            self.log.resp(
